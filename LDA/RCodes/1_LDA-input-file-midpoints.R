## This code is to format the input files for to run LDA for Lampe F&F Study midpoint data
## Last edited by Destiny Mullens December 2, 2024
library(dplyr)
library(tibble)
## Import & Cleanup Normalized counts
counts <- read.csv("../Working_Data/Counts/Norm-Counts-midpoints-73percent-20cpm-12022024.csv") %>% remove_rownames %>% column_to_rownames(var = "X")
...
write.csv(LDA.key,"Input_Files/LDA-key-LFF-midpoints-12022024.csv",row.names = FALSE)
...
counts <- left_join(meta,rownames_to_column(counts),by=c("Sample.Name"="rowname")) ## Join with metadata to make sure rows are in correct order
                                                                                   ## All rows from group A2 & B2 should be grouped together
labels <- counts[,1:2]
...
fp <- c("Input_Files")
## File name
filename <- paste(fp,"LFF-midpoints-12022024.txt",sep = "/")
## Adds the numbers at the top of input file indicating the number of samples.
sample.count <- paste(length(which(labels$Group.TP=="A2")),",",length(which(labels$Group.TP=="B2")),sep = "")
